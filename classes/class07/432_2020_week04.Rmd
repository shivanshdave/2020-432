...
We want to build a model to effectively predict our outcome `yrs_lost_rate` using the 10 quantitative candidate predictors we've identified. (We'll tackle the categorical predictors separately, later.)
...
Predict `yrs_lost_rate` using the 12 candidate predictors.
...
## I'm going to go with the logarithm
...
Let's focus for the moment on building a model to effectively predict our outcome `log_yrslost` using the 10 quantitative candidate predictors we've identified. (We'll tackle the categorical predictors separately, later.)
...
- The `leaps` package in R has a function called `regsubsets` to help us with this approach.
...
p1 <- ggplot(rs_winners, aes(x = inputs, y = r2, 
    ...
    geom_label(data = subset(rs_winners, 
                             r2 == max(r2)),
               aes(x = inputs, y = r2, label = round(r2,3)), 
...
p2 <- ggplot(rs_winners, aes(x = inputs, y = rss, 
    ...
    geom_label(data = subset(rs_winners, 
                             rss == min(rss)),
               aes(x = inputs, y = rss, label = round(rss,2)), 
               fill = "black", col = "white") +
    theme_bw() +
    scale_x_continuous() +
    labs(x = "# of regression inputs",
...
1. What happens when you include categorical variables into best subsets?
2. The Spearman $\rho^2$ plot as a way to identify potential variables to consider non-linear terms.
3. Regression Diagnostics for Linear Models
4. What if we want to predict a binary outcome? Can we use `bestglm`?
5. What if we want to include sampling weights?
6. What about multiple imputation?
